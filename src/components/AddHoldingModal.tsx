...
const AddHoldingModal = ({ portfolioId = 1, trigger }: AddHoldingModalProps) => {
  ...
  const addHoldingMutation = useMutation({
    mutationFn: (data: { stockSymbol: string; quantity: number; averagePrice: number }) =>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Total Value Display */}
          {quantity && averagePrice && !isNaN(parseFloat(quantity)) && !isNaN(parseFloat(averagePrice)) && (
            <Card className="bg-primary/5">
              <CardContent className="p-4">
                <div className="flex justify-between items-center">
                  <span className="font-medium">Total Value:</span>
                  <span className="font-semibold text-lg">
                    {formatPrice(parseFloat(quantity) * parseFloat(averagePrice))}
                  </span>
                </div>
              </CardContent>
            </Card>
